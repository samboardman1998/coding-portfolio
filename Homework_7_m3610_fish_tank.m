% set parameters:
nsim = 2000; % number of different simulations to do
...
fixed_delivery = 0; % >0 ==> standing order for a new tank
                    % every so many days
...
argmaxexpected_delivery = 0;
maxexpected_delivery = -inf;

argminstd_delivery = 0;
minstd_delivery = inf;
 ...
 for sim=1:nsim
    ...
    for week = 1:nweeks
        for weekday = 1:7
            ...
            %This line was a bug in the UW LectureNotes implementation!
            %if (mod(7*week + day, fixed_delivery) == 0)
            if (mod(day, fixed_delivery) == 0)
...
if(average_profit > maxexpected_delivery)
    
    maxexpected_delivery = average_profit;
    argmaxexpected_delivery = i;

end

if(standard_deviation < minstd_delivery)
    
    minstd_delivery = standard_deviation;
    argminstd_delivery = i;
    
end

end

disp(['Value of fixed_delivery that maximizes estimated expected profit:' sprintf('%10.2f', argmaxexpected_delivery)])
disp(['Value of fixed_delivery that minimizes estimated standard deviation of profit:' sprintf('%10.2f', argminstd_delivery)]) 
